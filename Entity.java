...
public abstract class Entity {
  ...
  public static double TIME_SCALE = 100;     
 ...
 /** maximum speed for a player for use in tournament */
  ...
  public final void updatePosition (long time, Field field, int id) throws EntityOutOfBoundsException{
    ...
    if(this.x < field.minX || this.x > field.maxX || this.y < field.minY || this.y > field.maxY){
  ...
  public void status(Field field){}
  
    public void move(Field field, double x, double y, double speedFactor){    
      ...
      if (xVector == 0){// dont need to move on x axix
        ...
        if (yVector >= 0 ){// check Y vector is positve or negative
          this.speedY = speedFactor * MAX_SPEED;//positiver max speed
        }
        else{
          this.speedY = speedFactor * -MAX_SPEED;// negative max speed
        }
        ...
        if (Math.abs(xVector) < Math.abs(yVector)){// check which direction has bigger magnitute
          ...
          if (xVector > 0 ){
            this.speedX = speedFactor * MAX_SPEED;
          }
          else{
            this.speedX = speedFactor * -MAX_SPEED;
          }
    ...
    public boolean isPlayerInField(Field field){
      int xMin = field.minX;
       int xMax = (field.maxX - field.minX)/2;
       if (field.getTeam2().contains(this)){
         xMin = (field.maxX - field.minX)/2;
         xMax = field.maxX;
       }
      if ((x >= xMin && x < xMax + 3)){
         return true;
     ...
     public boolean isEntityInTerritory(Field field) {
       int xMin = field.minX;
       int xMax = (field.maxX - field.minX)/2;
       if (field.getTeam2().contains(this)){
         xMin = (field.maxX - field.minX)/2;
       ...
       if ((x >= xMin && x < xMin + 3) || (x <= xMax && x > xMax - 17)
             || (y >= field.minY && y < field.minY + 3) || (y <= field.maxY && y > field.maxY - 17)){
         return true;
       }
       return false;

     }
     public boolean isEntityAtBorder(Field field) {
       if ((x >= field.minX && x < field.minX + 3) || (x <= field.maxX && x > field.maxX - 17)
             || (y >= field.minY && y < field.minY + 3) || (y <= field.maxY && y > field.maxY - 17)){
         return true;
       }
       return false;
     ...
     public void moveRandomlyWithinTerritory(Field field){
  // TODO Auto-generated method stub
       if ( this.isEntityAtBorder(field) || this.isEntityInTerritory(field)){
         this.speedX *= -1;
         this.speedY *= -1;
       } 
       else {
