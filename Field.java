...
public class Field{
  ...
  protected ArrayList<Player> team1 = new ArrayList<Player>();
  
  /** the team for territory 2 */
  protected ArrayList<Player> team2 = new ArrayList<Player>();
  
  /** getter for the players in team 1 
    * 
    * @return the players in team 1
    */
  public ArrayList<Player> getTeam1(){ return this.team1; }

  /** getter for the players in team 2 
    * 
    * @return the players in team 2
    */
  public ArrayList<Player> getTeam2(){ return this.team2; }
  ...
  protected ArrayList<Entity> team1JailedPlayers = new ArrayList<Entity>();
  protected ArrayList<Entity> team2JailedPlayers = new ArrayList<Entity>();
  
  public ArrayList<Entity> getTeam1JailedPlayers() {
    return team1JailedPlayers;
  }

  public ArrayList<Entity> getTeam2JailedPlayers() {
    return team2JailedPlayers;
  }
  ...
  public boolean catchOpponent(Player a, Player b){
    ...
    if(b.side==1 && b.getX() < (this.maxX-this.minX)/2){
      return false;
    }
    if(b.side==2 && b.getX() > (this.maxX-this.minX)/2){
      return false;
    }
  ...
  public boolean freeTeammate(Player a, Player b) {
    ...
    if (Math.hypot(a.getX() - b.getX(), a.getY() - b.getY()) <= ARMS_LENGTH) {
      ...
      if (getTeam1().contains(a)) {
        getTeam2JailedPlayers().remove(b);
      } 
      else {
        getTeam1JailedPlayers().remove(b);
      }
  ...
  public void status(){
    ...
    this.flag1.status(this);
    this.flag2.status(this);
  }
  
  public boolean isEntityAtBase(Entity entity) {
    int[] basePosition = this.getBase1Position();
    if (getTeam2().contains(entity)) {
      basePosition = this.getBase2Position();
    }
    if (Math.hypot(entity.getX() - basePosition[0], entity.getY() - basePosition[1]) <= ARMS_LENGTH) {
      return true;
    }
    return false;
  }
