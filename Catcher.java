    ...
    public void play(Field field){
      ...
      if ( field.team1.contains(this)){
        if ( field.team2.size()==0){
          ...
          target = field.team2.get(this.targetInt);//makes shallow copy of the target
        }
      }
      else{
        if ( field.team1.size()==0){
          ...
          target = field.team1.get(this.targetInt);//makes shallow copy of the target
        }
      }
      if (target== null){
        this.speedX = 0;
      ...
      // direction needed to reach target
        ...
        if (xVector == 0){// dont need to move on x axix
          ...
          if (field.catchOpponent(this, (Player)target)==true){
            this.targetInt++;
            if ( field.team1.contains(this)){
              if (this.targetInt> field.team2.size()-1){
                this.targetInt = 0;
              }
            }
            else{
              if (this.targetInt> field.team1.size()-1){
                this.targetInt = 0;
              }
